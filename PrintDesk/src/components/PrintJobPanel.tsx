import { Clock3, Flag, Pause, Play, RotateCcw, Square, X } from "lucide-react";
...
interface Props { gcode: ParsedGcode | null; progress: PrintProgress; connected: boolean; status: PrinterStatus; isTestMode: boolean; canStartPrint: boolean; canStartTestPrint: boolean; onStartPrint(): void; onStartTestPrint(): void; onPause(): void; onResume(): void; onStop(): void; onReset(): void; preflightIssues: PreflightIssue[]; activeProfile: PrinterProfile; cancelledObjectIds: string[]; onCancelObject(protocol: NonNullable<ParsedGcode["objectCancellationProtocol"]>, objectId: string): void }

export default function PrintJobPanel(props: Props) {
  const { gcode, progress, connected, status, isTestMode, canStartPrint, canStartTestPrint, preflightIssues, activeProfile, cancelledObjectIds, onCancelObject } = props;
