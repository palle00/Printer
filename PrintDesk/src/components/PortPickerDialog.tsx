...
export default function PortPickerDialog({
  ...
  const connect = async (): Promise<void> => {
    ...
    try {
      ...
      await onConnect(selectedPath, profile?.baudRate ?? 115200);
      onClose();
  ...
  return (
    ...
    >
      ...
      >
        <header className="flex items-start justify-between border-b border-gray-800 px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            <span className="font-mono text-xs text-gray-500">
              {profiles.find((profile) => profile.id === activeProfileId)?.baudRate ?? 115200} baud
            </span>
            <button
              type="button"
              onClick={onClose}
        ...
        <div className="p-5">
          <label className="mb-4 block text-xs font-bold uppercase text-gray-400">
            Printer profile
            <select value={activeProfileId} onChange={(event) => onSelectProfile(event.target.value)} className="mt-2 block w-full border border-gray-700 bg-[#0f131d] px-3 py-2 text-sm font-normal normal-case text-gray-200">
              {profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} - {profile.baudRate} baud</option>)}
            </select>
          </label>
