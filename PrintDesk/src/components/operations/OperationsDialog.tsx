...
export default function OperationsDialog(props: Props) {
  ...
  return (
    <div className="absolute inset-0 z-[96] grid place-items-center bg-black/80 p-4" onMouseDown={(event) => event.target === event.currentTarget && props.onClose()}>
      <section role="dialog" aria-modal="true" aria-label="Operations" className="flex h-[min(760px,92vh)] w-[min(1000px,96vw)] flex-col overflow-hidden border border-gray-700 bg-[#121620] shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {tab === "activity" && <ActivityTab settings={props.settings} onChange={props.onChange} onOpenQueued={props.onOpenQueued} />}
