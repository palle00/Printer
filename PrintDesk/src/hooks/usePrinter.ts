...
export function usePrinter() {
  ...
  const connect = useCallback(async (path: string, baudRate = 115200) => {
    try {
      const connection =
        await window.desktop.printer.connect(path, baudRate);
  ...
  const cancelObject = useCallback((protocol: ParsedGcode["objectCancellationProtocol"], objectId: string) => {
    if (!protocol) return;
    void window.desktop.printer.cancelObject(protocol, objectId).catch(reportError);
  }, [reportError]);
